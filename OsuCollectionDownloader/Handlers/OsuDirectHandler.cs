...
internal sealed class OsuDirectHandler(HttpClient client) : IMirrorHandler
{
    public async Task<Result<bool>> DownloadAsync(string filePath, int beatmapSetId, CancellationToken token)
    {
        try
        {
            using HttpResponseMessage response = await client.GetAsync($"https://api.osu.direct/d/{beatmapSetId}", HttpCompletionOption.ResponseHeadersRead, token);
